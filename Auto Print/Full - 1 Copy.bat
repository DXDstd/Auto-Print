...
for  %%i in (*.pdf) do (
@echo        - Dang in %%i...
pdfprint  -printer "%printername%" -chgbin %bin2selected% -firstpage 1 -lastpage 1 -duplex 1 -copies 1 -collate 1 -quiet -useembedfont  "%%i"
)
@echo.
@echo.
@echo   2. Bam phim bat ky de in cac trang con lai cua phieu ket qua...
if %automatic%==0 (
	pause>nul
)
for  %%i in (*.pdf) do (
@echo        - Dang in %%i...
pdfprint  -printer "%printername%" -chgbin %bin2selected% -firstpage 2 -lastpage -1 -duplex 1 -copies 1 -collate 1 -quiet -useembedfont "%%i"
)
